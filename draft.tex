...
Pixels-based methods are generally based on low-level features, such as color and gradients, these methods are computationally efficient and easy to deploy. However, they demonstrate a poor performance in modelling some challenging scenes, like illumination changing and intermittent object motion.
The most popular approaches are Gaussian Mixture Model (GMM) and its improved methods, which utilize a mixture of weighted Gaussians to model the probability distribution of each pixel over time. 
Mingliang Chen et al. proposed a Spatiotemporal GMM algorithm, which combines the GMM with constrains of temporal and spatial information from the optical flow and superpixels. 

. 
Each pixels are assumed to be independent from their neighbors. 


%
low-level features (e.g. color intensities, gradients pixel-based

PBAS [10] and ViBe+ [14], follow this paradigm as they are both based on stochastic intensity sampling.
Parametric approaches based on Gaussian Mixture Models (GMM) [15] or non-parametric ones based on Kernel Density Estimation (KDE)  
%
