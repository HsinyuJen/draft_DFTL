...
Traditional foreground detection algorithms focus on anaylsing the pixel variation, establishing background models with statistical methods such as GMM and KDE.
...
%GMM 
% Ren:这里你说over time 不专业，但是论文里用得挺多的啊，表示随时间推移。还有那一句 pixels are .... to be if there exists ....， 我也是在其他论文里借过来用的。i
% 原文是Incoming pixels from video frames are labeled as background if there exists a Gaussian in the GMM, where the distance between its mean and the pixel lies within a certain bound. 
...
% 注意英语和汉语的表达习惯
% The most popular pixels-based methods are Gaussian Mixture Model (GMM) methods\cite{Stauffer1999}, 
which utilize a mixture of weighted Gaussians to model the probability distribution of each pixel in time sequence.
%over time. 
...
%
% Ren: 这里你说看不懂，我的意思是想说，GMM+里面参数会自动更新。并且每一个像素位置的高斯个数也会自适应调整。原文里说的是： Recursive equations are used to constantly update the parameters and but also to simultaneously select the appropriate number of components for each pixel.
% An improved GMM method is introduced by Zivkovic\cite{Zivkovic2004} with the utilization of recursive equations, 
% which can 
% automatically update the parameters and adjust the needed number of components of mixture for each pixel. 
%
%codebook
%Another popular algorithm in pixel based category is based on the Codebook. 
%Ren: 干脆删掉这一句了。。。
%Ren: 下面也重新写了。
...
Then incoming pixels are compared with these codewords to see if their distances lies within a certain bound. 

%KDE
% Ren这里确实没写好，因为有点看不太明白KDE.
A non-parametric background model is proposed in \cite{Elgammal2000Non}. 
Authors assume that each background pixel is drawn from a probability distribution function, which is estimated with Kernel Density Estimation(KDE). 
%Vibe
% Ren: 关于这个Another我觉得没问题啊，因为之前一句是讲非参数的方法KDE,这里说另一个非参数的方法我觉得没毛病啊。
% Ren: 还有 when there exists 这个短语确实没毛病啊，搜谷歌学术里这么用的也很多啊。
...
%PBAS
%
% Ren :这里上下两句话里，我用了一个R，一个R(X)，这两个东西是有关系的，ViBe里面要看距离是不是小于R, 下面这个论文是看距离是不是小于R(X),也就是说每个像素点位置，这个R是不一样的。
%
...
% Ren: 下面这一段，你说不知所云，我的意思是，之前的方法大多只考虑了像素值分布的概率，而不管它们变化过程中的顺序性。
% cqzhao: 不能这么说，实际上专注于distribution的方法不多。
% On the other hand, most pixels-based methods are only concerned with pixels distribution, 
%
%and ignore the order information of input frames by default.
...
%所以我们把像素整合到一起
% In this case在google scholar里也查得到很多啊
...
The benefit of bringing together the observations is to ensure the data integrity and preserve the temporal coherence of our training data. And that guarantee us the ability to take advantage of the spatial-temporal context. 
%所以我们方法比基于像素的要好。
...
which is derived from expectation maximization theory with the consideration of neighboring pixels while generating the model of the observed scene.
% Spatiotemporal GMM
%
% 妈蛋。居然怀疑我没看过，我看过这篇论文啊，用最小生成树组织了层次化的superpixels,用来做spatial constrains我下面不是提了的吗，只是更抽象一点。。。
In addition, 
...
%Another GMM based method named Spatiotemporal GMM algorithm was proposed in \cite{2017_TPAMI_GANGWANG}. 
%
%The authors combine the GMM with constrains of temporal and spatial information from the optical flow and hierarchical superpixels.
%(Bayesian Modeling)
Moreover, 
...
Similarly, in \cite{Reddy2010Robust}, origin images are divided into overlapping blocks. Each block is sequentially processed by an adaptive multi-stage classifier, which consists of a likelihood evaluation, an illumination invariant measure and a temporal correlation check.
...
But we also notice that some approaches are involved with optical flow field, which can be viewed as a limited temporal coherence message. 
%但是他们也有几个缺点
...
The last category of background subtraction methods apply traditional machine learning and deep learning on different features for the background modeling.
...
%
% Ren: 这个CNN是cascade的结构，就是个层次化预测结果的组合。
...
%       所以我这里一连提了四个这篇论文训练要采集的patches。 patches 分别来自 平均背景，原始图像，真实图，或者其他算法算出的真实图。
...
However, due to the diversity and complexity of nature scenes, it could be rather difficult to analyze the pixel variations for classification. 
...
% cqzhao: 怎么就是一个输入不同呢？吹的太低的，给我拔高！
Different with their methods, our network is trained to learn the patterns of pixel variations and transform them into a new space for classification.
% 我们的输入XXX
The input of our network is not the origin images or patches. Instead, we regard the pixel variation as a whole for the network training to preseve the temporal coherence between the pixels' historical observations.
%因为我们的方法是基于variation transformation建模的，所以我们的模型在时序建模上有更大的潜力。
% cqzhao: 我们的方法是variation transformation，只是相比他们时许好点而已，不是说我们就只是个时序，吹的不够，拔高！
Now, since the proposed approach is based on the variation transformation, our DPVTL model has more potential in time series modeling on each pixel position.
%此外，我们还将pixel variation 与其空间上的近邻连接起来，从而利用空间上下文信息。
% cqzhao: 这句逻辑有点混乱，我感觉连不上。
In addition, we also concatenate the pixel variation with its spatial neighbors to take advantage of the spatial context. 
%因为我们的网络没有借助于其他传统的背景建模方法，所以取得的效果应该归功于Variation Transformation 框架。
% cqzhao: 你说我们的网络结构简单干嘛》不是不打自招？？？往死里吹，！！！
Our DPVTL model is implemented without any assistance from other tradition background modeling methods, hence the reliable performance owing to the framework of variation transformation. 
