...
%
In this paper, we focus on transforming the variation into another space where the pixels are easier classified,
...
%
The fully convolutional network is trained to learn the pattern of variation for transformation with the output leaded by grounthtruth data.
% XXX 这句话还是有点问题
Benefied from the ingenious utilization of FCN leading by our clear congnition of essence about background subtractoin problem,
...
%
Then, a Fully Convolutinal Network (FCN)\ \cite{Shelhamer2017fcn} is applied to learn the patterns of the pixel variation and find a transformation which guarantee the linear separability of transformed pixel variation generated by mapping in a new space.
%
In particular,
...
It is notable that the quantity of observation sequences is exactly the resolution of the given video, which means an abundance of trainiing data can be obtained with just one image stack. 
since the pixel sequences are extracted from individual pixel positions, a large number of pixel matrixes may be extracted from only a single image stack, which promises that sufficient training data can be obtained with limited groundtruth.
...
Pixel observations are put into the $d\times d$ matrix according to the order of top to bottom and left to right. 
...
In order to benefit from the spatial context, we cascade the pixel matrix with 3 neighboring pixel matrixes randomly selected in 8 nearest neighbors, as shown in \reffig{flow_chart}. 
Now, we have the input of our network, which size is $d\times d\times 4$. 	


In the previous steps, videos are broken down into pixel matrixes which containing abundant background information. 
Next, the groundtruth matrixes are obtained in the same way, except the cascading process. 
...
In order to make output the same size as input, we borrowed ideas from Image Semantic Segmentation \cite{Shelhamer2017fcn}, which is doing padding to the input before the training. 
