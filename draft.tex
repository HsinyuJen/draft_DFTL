...
In particular, the pixel variation is represented by the sequence of pixel observations, which is used as the input of fully convolutional network to find a transformation of pixels' variation.
...
Traditional foreground detection algorithms focus on analyzing the pixel variation, establishing background models with statistical methods such as GMM\cite{Stauffer1999} and KDE\cite{Elgammal2000Non}.
...
%
Recent years, deep learning methods begin to flourish in several computer vision fields. A novel approach for foreground detection with the use of the Convolutional Neural Network(CNN) is proposed by Wang et al.\cite{wang2016PRL}.
...
We explain the production of the pixel patch, which is a specific form of the pixel variation, and the architecture of our network. 


The flow chart of the proposed approach is illustrated in \reffig{flow_chart}. 
...
In order to keep each piece of variation the same length, image stacks are sampled from the given videos before the pixel variations are extracted. 
Let denote the frames sequence as $\{ I_1,I_2,I_3,\dots,I_L\}$.
And image stacks can be defined as follows: 
\begin{equation}
    \begin{aligned}
&Stack_{(x)}=\{I_x,I_{(x+p)},I_{(x+2p)},\dots,I_L \},\\
&\quad p\times l=L,\quad 1\leq   x\leq p , 
    \end{aligned}
\end{equation}
% 解释采样的细节
...
I = C(H^m, SM_1, SM_2),
\end{equation}
where the $C$ represents the concatenation process on the third dimension.
...
Different with CNNs, FCNs utilize convolutional layers with $1\times 1$ kernels to take the place of fully connected layers, which provide several benefits. 
First and foremost, the size of output is adjustable in FCNs, which allows an end-to-end mapping of input patches and prediction patches at each pixel position. 
