...
and a novel background subtraction method called Deep Variation Transformation (DVT) is proposed.
%
In the DVT model,
...
%我们的不同在与输入
Different with their methods, the input of our DPVTL model is not the origin images. Instead, we take the pixel variation as the input to preseve the temporal coherence between the pixels.
...
%此外，因为我们的网络结构相对比较简单，所以取得的效果应该归功于时序相关性带来的辅助。
In addition, Our FCN is implemented with a relative simple structure, hence the reliable performance of our DPVTL should be owing to the enhanced temporal coherence. 
