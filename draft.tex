...
%背景检测的背景介绍
Background subtraction or foreground detection, as a fundamental problem in computer vision\ \cite{Bouwmans201431}, has been discussed over decades with the increasing number of outdoor cameras.
%
It is widely used as the pre-processing step of video processing, which can help us efficiently mark the region of interest, e.g. vehicles and humans, thus saving us huge amount of computing resources\cite{Barnich2011_2011_TIP}.
...
%
Currenrly, plenty of previous work focus on anaylsing the pixels' variation for background subtraction,
since the background subtraction is typically recognized as a binary classification that assigns each pixel in a video sequence with a label, for either belonging to the background or foreground scene. 
%
However, due to the unpredictability and diversity of the pixels' variation in natural scenes,
it is still a challenging problem to propose a good background subtraction in all complexly natural scenes.
...
In the DVTL model, a Fully Convolutinal Network(FCN)\cite{Shelhamer2017fcn} is applied to learn the patterns of the pixel variation and find a transformation which guarantee the linear separability of transformed pixel variation generated by mapping in a new space, as shown in \reffig{idea}. The final prediction can be obtained by thresholding of the transformed variation. 
\todo{provide more details about our method, the details should be enough to summarize proposed approach.}
...
%codebook
Another popular algorithms in pixel based category are based on Codebook. In \cite{Kim2005}, Kim et al present the codebook method to record the sampling background values at each pixel, which can be seemed as a compressed representation of background model. The final foreground is detected by a distance measurement in a cylindrical color model.
%KDE
...
However, Those assumptions of pixels' independence are incorrect or inapplicable to many real-world scenes. Hence they demonstrate a comparatively poor performance in modelling some challenging scenes, like illumination changing and intermittent object motion.
There is, in fact, a strong temporal coherence in image sequences that provides abundant hidden messages for the background model. 
The proposed approach take advantage of the temporal coherence, and that is the reason that we have a relative better performance in challenging scenes.  
...
We also notice that some methods are involved with optical flow field, which can be viewed as a limited temproal coherence message. 
However, optical flow methods have some shortcommings like the high computation complexity, moreover they are very sensitive to illumination changing. 
Compared with them, the proposed approach are more powerful in capturing the structural background variation and more efficient in computation due to the application of deep learning.
...
The authors also proposed a temproal approach to sample training images from the given video, thus providing the background model with limited temporal information. 
...
Different with their methods, with a relative simple structure of the network, the proposed approach is based on the time series modeling of each pixel. Hence the reliable performance of DVTL should be owing to the enhanced temporal coherence. 
...
DeepBS\cite{Babaee2017deep}      & 0.9580   & 0.8761  & \textbf{0.8990}     & 0.6097    & 0.9304 & 0.7583  & 0.8647   & 0.5900     & 0.6359     & 0.3306 & \textbf{0.8993}  & 0.7458  \\
